        ...
        <div v-if="Object.keys(news).length">
            <div class="mt-4">
                <a-breadcrumb class="mt-4">
                    ...
                    <a-breadcrumb-item>
                        <RouterLink :to="`/news/${route.params.id}`">{{ news.title }}</RouterLink>
                    </a-breadcrumb-item>
                </a-breadcrumb>
            </div>
                    ...
                    <a-col :xs="{ span: 24 }" :md="{ span: 7 }">
                        <div class="shadow  news-accardion">
                            ...
                            <a-collapse v-model:activeKey="activeKey" ghost>
                                <a-collapse-panel key="1" class="accardion-item"
                                    header="СРАВНЕНИЕ ЯПОНСКИХ И НЕМЕЦКИХ АВТОМОБИЛЕЙ: ЧТО ЛУЧШЕ?">
                                    <p>{{ text }}</p>
                                </a-collapse-panel>
                                <a-collapse-panel key="2" class="accardion-item"
                                    header="ТОП-5 НАДЕЖНЫХ АВТОМОБИЛЕЙ ДЛЯ СЕМЕЙНОГО ИСПОЛЬЗОВАНИЯ">
                                    <p>{{ text }}</p>
                                </a-collapse-panel>
                                <a-collapse-panel key="3" class="accardion-item"
                                    header="ТОП-5 НАДЕЖНЫХ АВТОМОБИЛЕЙ ДЛЯ СЕМЕЙНОГО ИСПОЛЬЗОВАНИЯ">
                                    <p>{{ text }}</p>
                                </a-collapse-panel>
                            </a-collapse>
                            <a href="#" class="new-addition mt-4">Читать больше новостей <i
...
const fetchNew = async () => {
    ...
    error.value = null
    try {
        const response = await axios.get(`${import.meta.env.VITE_APP_API}/news/${route.params.id}`)
